...
public class CoolstoreSetps {
	...
	public void the_shipping_cost_is(double shippingCost) throws Throwable {
		Assert.assertTrue(String.format("The expected cost was $%s, but the actual cost was $%s", shippingCost  ,shoppingCart.getShippingTotal().doubleValue() ),shoppingCart.getShippingTotal().doubleValue() == shippingCost);
	}
